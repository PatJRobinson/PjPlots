...
namespace PjPlot {
    ...
    namespace detail {
        ...
        template <Size2 Size>
        static void clear(Img2<Size>& img_data) {
            for (auto& row : img_data) {
                for (auto& pixel : row) {
                    pixel.set(0, 0, 0, 0);
                }
            }
        ...
        template <Size2 Size>
        static void bresenham_line(Img2<Size>& img_data, Colour line_colour, size_t x1, size_t x2, size_t y1, size_t y2) {
            
            // assert (x1, y1) and (x2, y2) lie within img_data, throw if now

            const bool slope = abs(x2 - x1) > abs(y2 - y1);
            ...
            if (x1 > x2) {
            const auto dx1 = abs(x2 - x1);
            const auto dy1 = abs(y2 - y1);
                std::swap(x1, x2);
                std::swap(y1, y2);
            }
            ...
            for (size_t x = x1; i < x2; ++x) {
                auto& px = slope ? img_data[y][x] : img_data[x][y];
                px.set(line_colour);
                error -= dy;
                if (error > 0) {
                    y+=ystep;
